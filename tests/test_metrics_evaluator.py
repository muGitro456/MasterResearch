...
_METRICS_EVALUATOR_PATH = os.path.join(os.path.dirname(__file__), '..', 'tools', 'metrics_evaluator.py')


class TestMetricsEvaluatorCLI:
    def _run(self, argv, mocker, extra_patches=None):
        ...
        runpy.run_path(_METRICS_EVALUATOR_PATH, run_name='__main__')

    def test_normal_rni_first_better(self, mocker, capsys):
        mocker.patch.object(sys, 'argv', ['metrics_evaluator.py', '--rni'])
    ...
    def test_normal_val_option(self, mocker):
        mocker.patch.object(sys, 'argv', ['metrics_evaluator.py', '--val'])
    ...
    def test_normal_rniall_option(self, mocker):
        ...
        mocker.patch.object(sys, 'argv', ['metrics_evaluator.py', '--rniall'])
    ...
    def test_abnormal_no_args(self, mocker, capsys):
        mocker.patch.object(sys, 'argv', ['metrics_evaluator.py'])
        runpy.run_path(_METRICS_EVALUATOR_PATH, run_name='__main__')
        captured = capsys.readouterr()
        assert "オプション" in captured.out

    def test_abnormal_invalid_option(self, mocker, capsys):
        mocker.patch.object(sys, 'argv', ['metrics_evaluator.py', '-unknown'])
        runpy.run_path(_METRICS_EVALUATOR_PATH, run_name='__main__')
        captured = capsys.readouterr()
