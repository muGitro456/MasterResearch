...
def fpo_mopso_master(params, multiObj):
    # パラメータ設定
    N = params[0] # 粒子数
    ...
    # トポロジーの初期化
    TOPOLOGIES = init_topologies(N, N_SIZE)
    #print(TOPOLOGIES)
    ...
    ARC_SUB = [None] * N
    for i in range(N):
        ARC_SUB[i] = init_subArchive(POS, POS_EVAL, TOPOLOGIES[i])      
    #print(type(ARC_SUB))
    ...
    while not stopCondition:
        ...
        VEL = W * VEL + C1 * np.random.rand(N, D) * (PBEST - POS) \
                        + C2 * np.random.rand(N, D) * (LBEST - POS)\
        ...
        # サブアーカイブの更新
        for i in range(N):
            ARC_SUB[i] = update_sub_archive(ARC_SUB[i], POS, POS_EVAL, TOPOLOGIES[i])
        # サブアーカイブを一つに統合する
...
def init_topologies(N, N_SIZE):
    ...
    for i in range(N):
        for m in range(N_SIZE):
            topologies[i, m] = (i + idx_particle[m - int((N_SIZE - 1) / 2)]) % N
    return topologies

# topology:1行N_SIZE列の行列
def init_subArchive(pos, posEval, topology):
    subArc_pos = np.empty((len(topology), pos.shape[1]))
    subArc_posEval = np.empty((len(topology), posEval.shape[1]))
    
    for i in range(subArc_pos.shape[0]):
        subArc_pos[i, :] = pos[int(topology[i]), :]
        subArc_posEval[i, :] = posEval[int(topology[i]), :]
...
def update_sub_archive(subArc, pos, posEval, topology):
    subFront = init_subArchive(pos, posEval, topology)
